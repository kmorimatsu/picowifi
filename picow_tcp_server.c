...
static const char g_response[]=
	...
	"<body>Hello TCP server world!</body>"
	"</html>"
;
...
err_t tcp_server_send_data(void *arg, struct tcp_pcb *tpcb)
{
    ...
    for(int i=0; i< (sizeof g_response)-1; i++) {
        //state->buffer_sent[i] = rand();
        state->buffer_sent[i] = g_response[i];
    }
    ...
    err_t err = tcp_write(tpcb, state->buffer_sent, (sizeof g_response)-1, TCP_WRITE_FLAG_COPY);
    if (err != ERR_OK) {
